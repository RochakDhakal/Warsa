 ...
 *
 * @example
 * warsa_get_feature_status_icon( true );  // Outputs a check mark icon.
 * warsa_get_feature_status_icon( false ); // Outputs a cross icon.
 * warsa_get_feature_status_icon( 'Coming Soon' ); // Outputs "Coming Soon" as a text status.
 */
function warsa_get_feature_status_icon( $status ) {
	$allowed_tags = array(
		'div'  => array(
			'class' => true,
		),
...
<div class="warsa-page__free-pro">
	<h3><?php esc_html_e( 'Go with Cozy Blocks PRO', 'warsa' ); ?></h3>

	<div class="checkout-button-container top">
		<a href="<?php echo esc_url( 'https://cozythemes.com/pricing-and-plans/' ); ?>" class="checkout-button"><?php esc_html_e( 'Already enjoying our product?', 'warsa' ); ?></a>
	</div>

	<table>
		<tr>
			<th><?php esc_html_e( 'Feature', 'warsa' ); ?></th>
			<th><?php esc_html_e( 'Free', 'warsa' ); ?></th>
			<th><?php esc_html_e( 'Pro', 'warsa' ); ?></th>
		</tr>

		<?php
		if ( isset( $warsa_features ) && ! empty( $warsa_features ) ) :
			foreach ( $warsa_features as $feature ) {
				?>
				<tr>
					<td>
						<?php
					...
					<td>
						<?php warsa_get_feature_status_icon( $feature['free'] ); ?>
					</td>

					<td>
						<?php warsa_get_feature_status_icon( $feature['pro'] ); ?>
					</td>
				</tr>
				<?php
			}
	...
	<br />
	<p><?php esc_html_e( 'Explore More features and details comparison on ', 'warsa' ); ?>
		<a href="<?php echo esc_url( 'https://cozythemes.com/warsa/' ); ?>" target="_blank"><?php esc_html_e( 'Here', 'warsa' ); ?></a>
	</p>
	...
	<div class="refund-policy">
		...
		<div class="policy">
			<h3><?php esc_html_e( 'No Risk, 100% Money Back Guarantee!', 'warsa' ); ?></h3>
			<p><?php esc_html_e( 'Our Refund Policy secures customer satisfaction and protection. If you find our purchased products unusable, despite our team’s support, you can claim a full refund within 30 days.', 'warsa' ); ?></p>
		</div>
	</div>

	<!-- Checkout Button Section -->
	<div class="checkout-button-container bottom">
		<?php esc_html_e( 'Ready to take the next step? Let’s get you to', 'warsa' ); ?> <a href="<?php echo esc_url( 'https://cozythemes.com/pricing-and-plans/' ); ?>" class="checkout-button"><?php esc_html_e( 'Checkout!', 'warsa' ); ?></a> <?php esc_html_e( '🚀', 'warsa' ); ?>
	</div>
</div>
<?php
